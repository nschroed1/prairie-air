...
Live URL: https://prairie-air.extremecode-767.workers.dev
...
4. Build with `npm run check:cloudflare` and deploy the generated configuration using `npx wrangler deploy --config dist/server/wrangler.json`. This command publishes publicly on the account's `workers.dev` subdomain.
5. Add the exact resulting `/auth` and `/auth?view=reset` URLs to Supabase's allowed redirects and make the chosen production origin its Site URL. Preserve the old origin's redirects during the transition. Add a custom domain when one is selected.
